...
// ══════════════════════════════════════════════════════
// REDIRECT PÚBLICO — WHATSAPP SIMULAÇÃO CLT (s9)
// GET /simular-clt -> pega número aleatório -> 302 -> wa.me
// Domínio: clt.nhcred.com
// ══════════════════════════════════════════════════════

const TABELA_NUMEROS = 'numeros';

const MENSAGEM = '(s9) Olá, Novo Horizonte! Quero simular o consignado CLT.';
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!numeros || numeros.length === 0) {
      console.log(`[WHATS-SIMULAR-CLT FALLBACK] Nenhum número cadastrado, buscando fallback ativo`);
    ...
    console.log(`[WHATS-SIMULAR-CLT REDIRECT] ${new Date().toISOString()} → ${sorteado.numero} → ${whatsappUrl}`);
    ...
    if (!req.query.test) {
      ...
      dispararWebhook('redirect.whats-simular-clt', {
        produto: 'Simular CLT',
        codigo: 's9',
        numero: sorteado.numero,
        ip: clientIp,
    ...
    console.error('[WHATS-SIMULAR-CLT ERROR]', err);
    const fb = await getFallbackNumber(TABELA_NUMEROS);
    return res.redirect(302, `https://wa.me/55${fb || '0'}${textParam}`);
  }
};
