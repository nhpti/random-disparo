...
// ══════════════════════════════════════════════════════
// REDIRECT PÚBLICO — RETOMADA FGTS (f02)
// ══════════════════════════════════════════════════════

// Usa a tabela principal do FGTS
const TABELA_NUMEROS = 'numeros';

// Nova mensagem com a tag (f02)
const MENSAGEM = '(f02) Olá Novo Horizonte! Quero sacar meu FGTS e receber agora!'
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!numeros || numeros.length === 0) {
      console.log(`[FGTS-F02 FALLBACK] Nenhum número cadastrado, buscando fallback ativo`);
    ...
    console.log(`[FGTS-F02 REDIRECT] ${new Date().toISOString()} → ${sorteado.numero} → ${whatsappUrl}`);
    ...
    if (!req.query.test) {
      ...
      dispararWebhook('redirect.fgts-f02', {
        produto: 'FGTS Retomada',
        codigo: 'f02',
        numero: sorteado.numero,
        ip: clientIp,
        url: whatsappUrl,
    ...
    console.error('[FGTS-F02 ERROR]', err);
    const fb = await getFallbackNumber(TABELA_NUMEROS);
    return res.redirect(302, `https://wa.me/55${fb || '0'}${textParam}`);
  }
};
