...
// ══════════════════════════════════════════════════════
// REDIRECT PÚBLICO — /disparointerno
// GET /disparointerno → pega número aleatório → 302 → wa.me
// ══════════════════════════════════════════════════════

const TABELA_NUMEROS = 'numeros';

// Mensagem pré-preenchida que aparece no WhatsApp
const MENSAGEM = '(di1) Olá Novo Horizonte! Quero saber mais sobre o Crédito do Trabalhador!';
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!req.query.test) {
      ...
      dispararWebhook('redirect.fgts-disparointerno', {
        produto: 'CLT & FGTS - Disparo Interno',
        numero: sorteado.numero,
        ip: clientIp,
        url: whatsappUrl,
      });
    ...
    console.error('[FGTS-DISPAROINTERNO ERROR]', err);
    const fb = await getFallbackNumber(TABELA_NUMEROS);
    return res.redirect(302, `https://wa.me/55${fb || '0'}${textParam}`);
  }
};
