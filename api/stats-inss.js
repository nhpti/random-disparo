...
// GET /api/stats-inss — estatísticas do produto INSS (autenticado)
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const [countTotal, countPeriodo, firstPage] = await Promise.all([
      supabase.from(TABELA_LOG).select('*', { count: 'exact', head: true }),
      supabase.from(TABELA_LOG).select('*', { count: 'exact', head: true })
        .gte('created_at', inicioFiltro).lte('created_at', fimFiltro),
      supabase.from(TABELA_LOG).select('numero, ip, created_at')
        .gte('created_at', inicioFiltro).lte('created_at', fimFiltro)
        .range(0, PAGE - 1).order('created_at', { ascending: true }),
    ]);

    if (countTotal.error && countTotal.error.code === '42P01') {
    ...
    console.error('[STATS INSS ERROR]', err);
    return res.status(500).json({ error: 'Erro ao buscar stats do INSS' });
  }
};
