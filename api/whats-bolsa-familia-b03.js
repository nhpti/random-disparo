...
// ══════════════════════════════════════════════════════
// REDIRECT PÚBLICO — DISPARO SERGIO BOLSA FAMÍLIA (b03)
// ══════════════════════════════════════════════════════

const TABELA_NUMEROS = 'numeros_bolsa_familia';

const MENSAGEM = '(b03) Olá! Quero saber mais sobre o empréstimo Bolsa Família!'
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!numeros || numeros.length === 0) {
      console.log(`[WHATS-BOLSA-FAMILIA-B03 FALLBACK] Nenhum número cadastrado, buscando fallback ativo`);
    ...
    console.log(`[WHATS-BOLSA-FAMILIA-B03 REDIRECT] ${new Date().toISOString()} → ${sorteado.numero} → ${whatsappUrl}`);
    ...
    if (!req.query.test) {
      ...
      dispararWebhook('redirect.whats-bolsa-familia-b03', {
        produto: 'Bolsa Família Disparo Sergio',
        codigo: 'b03',
        numero: sorteado.numero,
        ip: clientIp,
        url: whatsappUrl,
    ...
    console.error('[WHATS-BOLSA-FAMILIA-B03 ERROR]', err);
    const fb = await getFallbackNumber(TABELA_NUMEROS);
    return res.redirect(302, `https://wa.me/55${fb || '0'}${textParam}`);
  }
};
